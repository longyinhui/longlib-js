...
(function (window, document, undefined) {
    ...
    DataGrid.prototype = {
        ...
        _initEvents: function () {
            ...
            this.$head.bind("click", function (e) {
                ...
                if (nodeName == "th" || nodeName == "span") {
                    var th = (nodeName == "span") ? target.parentNode : target, thSiblings = th.parentNode.childNodes, i, isAsc = e.offsetY < (th.clientHeight / 2);
                    ...
                    for (i = 0; i < thSiblings.length; i++)
                        if (thSiblings[i].className)
                            thSiblings[i].className = _variables.sortThClass;
                    th.className = isAsc ? _variables.ascThClass : _variables.descThClass;
                }
                else if (nodeName == "input" && target.type.toLowerCase() == "checkbox") {
                    var i, value, rowCell, checkboxes, checkboxName;
                    cell = datagrid.allCells[target.parentNode.parentNode.id];
                    if (!cell) return;
        ...
        _load: function () {
            ...
            bodyHtml += this.id;
            bodyHtml += "' class='";
            bodyHtml += this._editable ? "" : (_variables.autoHeightClass + " ");
            bodyHtml += _variables.dataGridClass;
            bodyHtml += "'><thead>";
            bodyHtml += headRows;
        ...
        createNewRow: function (position, speed) {
            ...
            if (position == this.rows.length - 1) {
                ...
                //todo: need to investigate which one of above 2 methods is more effective
            }
            else{
        ...
        findRowsByWhere: function (where, firstN) {
            ...
            for (i = 0; i < this.rows.length; i++) {
                ...
                clause = clauseTemplate.replace(/\[/g, "document.getElementById(\"" + preCellID).replace(/\]/g, "\").innerText");
                if (eval(clause)) {
                    rowIndexes.push(i);
                    if (firstN && rowIndexes.length >= firstN) break;
                }
        ...
        _blurRow: function(){
            ...
            if (document.getElementsByClassName)
                trs = this.$.getElementsByClassName(_variables.focusTrClass);
            else if (document.querySelectorAll)
                trs = this.$.querySelectorAll("." + _variables.focusTrClass);
            else
        ...
        _focusRow: function(tr){
            this._blurRow();
            tr.className = _variables.focusTrClass;
        ...
        getFocusedRow: function () {
            var tr, td, row, cell;
            if (document.getElementsByClassName){
                tr = this.$.getElementsByClassName(_variables.focusTrClass)[0];
                if (tr) td = tr.firstChild;
            }
            else if (document.querySelector)
                td = this.$.querySelector("." + _variables.focusTrClass + " td");
            else{
                tr = this.$.getElementsByTagName("tr");
                for (var i = 0; i < tr.length; i++)
                    if (tr[i].className == _variables.focusTrClass) {
                        td = tr[i].firstChild;
                        break;
                    }
            }
        ...
        deleteRow: function (rowIndex, speed) {
            ...
            var datagrid = this, tr = this.getCell(rowIndex, 0).parentNode;
    ...
    DataGridCell.prototype = {
        ...
        _generateCell: function (parentRow, columnIndex) {
            ...
            if (parentRow.rowIndex < 0) {//Head
                ...
                cell += column.enableSorting ? " class='" + _variables.sortThClass + "'" : "";
                cell += this.id ? " id='" + this.id + "'" : "";
                cell += column.caption ? " title='" + column.caption + "'><span>" : "><span>";
                cell += (this.cellType == "checkbox" && column.selectall == 1) ? "<input type='checkbox' style='position:relative;top:3px;'>" : "";
                ...
                if ("checkbox,radio,image".indexOf(this.cellType) >= 0)
                    cellClass = _variables.contentTdClass;
                else if ("button,xmlform".indexOf(this.cellType) >= 0)
                    cellClass = _variables.inputTdClass;
                ...
                cell = "<td";
                cell += cellStyle == "" ? "" : " style='" + cellStyle + "'";
                cell += cellClass ? " class='" + cellClass + "'" : "";
                cell += this.id ? " id='" + this.id + "'>" : ">";
        ...
        _generateCellElement: function () {
            ...
            switch (type) {
                ...
                case "label":
                    ...
                    element = "<span";
                    element += this.cellValue ? " title='" + this.cellValue + "'>" : ">";
        ...
        _onClick: function () {
            var type = this.cellType, td = document.getElementById(this.id), input, label, cell = this, i, refresh = false, column;
            ...
            if (label && input) {
                td.className = _variables.inputTdClass;
                input.style.display = "";
                input.focus();
        ...
        _render: function (value) {
            ...
            switch (this.cellType) {
                ...
                case "select":
                    ...
                    td = document.getElementById(this.id);
                    if (td) label = td.getElementsByTagName("span")[0];
    ...
    document.writeln('<link rel="Stylesheet" href="' + window.getRootPath() + '/css/datagrid.2.0.css" />');
